...
int seleccionarReserva(sqlite3 *db, char usuario[20]) {
	...
	cout << "Usuario de la reserva: " << usuario << " Id Reserva: "
			<< seleccionId << " Dia: " << diaReserva2 << " Hora: "
			<< horaReserva2 << endl;
...
int eliminarReserva(sqlite3 *db, char usuario[20]) {
	...
	char sql[] = "SELECT ID_RESERVA, HORA, DIA, NUMERO_MESA FROM RESERVA WHERE USUARIO = ?";
	...
	do {
		...
		if (rc == SQLITE_ROW) {
			...
			horaReserva = sqlite3_column_int(stmt, 1);
			diaReserva = sqlite3_column_int(stmt, 2);
			numMesa = sqlite3_column_int(stmt, 3);
	...
	sqlite3 *db3;
	rc = sqlite3_open("e-restaurants.db", &db3);	//abrir base de datos
	if (rc != SQLITE_OK) {
		cout << "Error opening database" << endl;
		return rc;
	}
	...
	char sql3[] = "UPDATE MESA SET MESA_OCUPADA = '0' WHERE NUMERO_MESA = ?";
		...
		rc = sqlite3_bind_int(stmt3, 1, numMesa);
		if (rc != SQLITE_OK) {
			cout << "Error al bindear la consulta(UPDATE)" << endl;
			cout << sqlite3_errmsg(db) << endl;
